...
async fn main() {
    ...
    let finished_config_write=std::time::SystemTime::now();
    ...
    let finished_making_handles=std::time::SystemTime::now();

    println!("Config sent in {:?}",(finished_config_write.duration_since(start_time)));
    println!("Handled created in {:?}",(finished_making_handles.duration_since(start_time)));
