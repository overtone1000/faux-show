...
impl InternalService
{
    ...
    async fn handle_websocket_sink(external_command_receiver:Arc<Mutex<UnboundedReceiver<Command>>>, sink:Arc<Mutex<HashMap<u64,SplitSink<WebSocketStream<TokioIo<Upgraded>>,Message>>>>,)->()
    {
        ...
        loop {
            match external_command_receiver.recv().await
            {
                Some(command)=>{
    ...
    async fn handle_websocket(self, websocket: HyperWebsocket) -> () {       
        ...
        let external_command_receiver_clone = self.external_command_receiver.clone();
        ...
        let sink_key={
            ...
            match *current_handler
            {
                ...
                None => {
                    ...
                    *current_handler=Some(tokio::spawn(async move {Self::handle_websocket_sink(external_command_receiver_clone, sinksclone).await}))
                },
            };
